...
| `js/prefs.js` | Скрытые источники, категории, группы видеохостингов и языки (`ones-hidden-sources`, `ones-hidden-types`, `ones-hidden-source-groups`, `ones-hidden-languages`) |
...
| `sources.yaml` | Источники сборщика по секциям site / telegram / video (видео — вложенные группы; браузер не читает) |
...
```json
{
  "sources": [
    ...
    {
      "name": "Okolo_1C",
      "home": "https://www.youtube.com/@Okolo_1C",
      "source_type": "video",
      "group": "YouTube",
      "language": "ru"
    }
...
| `group` | string | Только у `video`: имя группы видеохостинга из ключа в `sources.yaml` (например `YouTube`). Сайты и Telegram поле не пишут |
...
6. Скрытые языки (`ones-hidden-languages`), источники (`ones-hidden-sources`), категории (`ones-hidden-types`) и группы видеохостингов (`ones-hidden-source-groups`) отфильтровываются **до** пикера и счётчиков. Фильтр «источник» **не** делает новых запросов. Пикер источника строится из уникальных `source_name` этого дня на выбранном языке, с заголовками по `source_type`; в раскрытом списке рядом с именем — число карточек. Плашка источника на карточке включает тот же фильтр. Метки `topics` на карточке только для чтения.
7. Поиск по заголовку и саммари (в памяти). Недавние запросы — `localStorage` ключ `ones-search-history`.
8. Клик по ссылке заголовка (и средняя кнопка мыши) помечает карточку прочитанной. Клик по бейджу «Прочитано» снимает отметку. Карта id: `ones-read` (до 500 записей, 90 дней).
...
10. Смена даты → новый fetch day-файла; строка поиска и фильтр источника сбрасываются; прокрутка — к началу дня. Язык и скрытые источники не сбрасываются. Смена языка → календарь перестраивается и открывается свежайшая дата этого языка. Ссылка `?lang=` включает этот язык в источниках (как пикер) и открывает день из его календаря.

`settings.html` («Настройки»): тот же пикер языка, дисклеймер, затем `GET data/sources.json` → группировка по `language`, внутри — по `source_type`, у видео ещё по `group`. Пустые языки, типы и группы скрываются. У языка — галочка на всю группу (`ones-hidden-languages`); по умолчанию включён только язык интерфейса, остальные можно отметить. Смена языка в шапке снова включает эту группу, чтобы лента не оказалась пустой. Галочка у категории (`ones-hidden-types`), у группы видеохостинга (`ones-hidden-source-groups`) или у канала (`ones-hidden-sources`) снимает их с ленты. На ленте шестерёнка открывает ту же панель оверлеем (дата и место просмотра не сбрасываются); `settings.html` остаётся для прямой ссылки. `about.html` перенаправляет сюда.
